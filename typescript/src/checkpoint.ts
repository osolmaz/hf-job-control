...
export type CheckpointResumeMode =
  "exact" | "boundary" | "restart" | "unsupported";
...
export interface CheckpointAdapter<
  RestoreEvidence extends Readonly<Record<string, JsonValue>>,
> {
...
export function checkpointBundleKey(
  prefix: string,
  runId: string,
  sha256: string,
): string {
  ...
  return joinPrefix(
    prefix,
    `${runId}/checkpoints/sha256-${sha256}/checkpoint.hfjob`,
  );
}

export function checkpointClaimPrefix(prefix: string, runId: string): string {
  requireSafeId(runId, "run_id");
  return joinPrefix(prefix, `${runId}/checkpoints/claims/`);
}

export function checkpointClaimKey(
  prefix: string,
  claim: Pick<CheckpointClaim, "run_id" | "attempt_id" | "sequence">,
): string {
  ...
  return joinPrefix(
    prefix,
    `${claim.run_id}/checkpoints/claims/sequence-${String(claim.sequence).padStart(16, "0")}/${claim.attempt_id}.json`,
  );
}

export function checkpointPointerKey(prefix: string, runId: string): string {
  requireSafeId(runId, "run_id");
...
export function createCheckpointBundle(options: {
  ...
  createdAt: string;
}): { bytes: Uint8Array; manifest: CheckpointManifest } {
...
export function verifyCheckpointBundle(
  bytes: Uint8Array,
  ...
  const manifest = parseCheckpointManifest(
    parseJson(
      source.subarray(manifestStart, manifestEnd),
      "checkpoint manifest",
    ),
  );
  ...
  for (const payload of manifest.payloads) {
    ...
    if (end > source.byteLength) {
      throw new Error(
        `checkpoint payload byte count mismatch: ${payload.path}`,
      );
    }
...
export function parseCheckpointManifest(value: unknown): CheckpointManifest {
  ...
  return {
    ...
    previous_checkpoint_sha256:
      record.previous_checkpoint_sha256 === null
        ? null
        : requireSha256(
...
export function parseCheckpointClaim(value: unknown): CheckpointClaim {
...
export function parseCheckpointPointer(value: unknown): CheckpointPointer {
...
export function parseCheckpointReceipt(value: unknown): CheckpointReceipt {
...
export function stableCheckpointJsonBytes(value: unknown): Uint8Array {
  return Buffer.from(
...
export class CheckpointCoordinator {
  ...
  readonly #store: CheckpointObjectStore;
  readonly #receiptStore: CheckpointReceiptStore | undefined;
  readonly #prefix: string;
  readonly #clock: () => Date;
  #head: { claim: CheckpointClaim; manifest: CheckpointManifest } | null = null;
  ...
  >(
    ...
    if (head === undefined) throw new Error("checkpoint chain head is missing");
  ...
  async commit<RestoreEvidence extends Readonly<Record<string, JsonValue>>>(
    boundaryValue: CheckpointBoundary,
    adapter: CheckpointAdapter<RestoreEvidence>,
    ...
    if (boundary.sequence !== expectedSequence) {
      throw new Error(
        `checkpoint boundary sequence must be ${String(expectedSequence)}`,
      );
    }
  ...
  async finish<RestoreEvidence extends Readonly<Record<string, JsonValue>>>(
    boundary: CheckpointBoundary,
    adapter: CheckpointAdapter<RestoreEvidence>,
  ): Promise<CheckpointReference> {
    const checkpoint = await this.commit(boundary, adapter);
    const head = this.#head;
    if (head === null) throw new Error("final checkpoint head is missing");
    await this.#publishReceipt({
      kind: "terminal",
      claim: head.claim,
      adapter: adapter.spec,
  ...
  async #loadChain(): Promise<readonly CheckpointClaim[]> {
    const keys = await this.#store.list(
      checkpointClaimPrefix(this.#prefix, this.#runId),
    );
    const claims: CheckpointClaim[] = [];
    for (const key of keys) {
      if (!CLAIM_PATH.test(`/${key}`)) continue;
      const raw = await this.#store.read(key);
      if (raw === null) throw new Error(`checkpoint claim disappeared: ${key}`);
    ...
    const sequenceNumbers = [...bySequence.keys()].sort(
    ...
    for (const [index, sequence] of sequenceNumbers.entries()) {
      ...
      if (
        candidates.some(
          (candidate) =>
            !equalReference(candidate.checkpoint, reference) ||
            candidate.previous_checkpoint_sha256 !== previousSha256,
        )
      ) {
        throw new Error(
          `conflicting checkpoint claims at sequence ${String(sequence)}`,
        );
      }
      const claim = candidates[0];
      if (claim === undefined) throw new Error("checkpoint claim is missing");
  ...
  async #readReference(reference: CheckpointReference): Promise<Uint8Array> {
  ...
  async #repairPointer(claim: CheckpointClaim): Promise<void> {
    ...
    try {
      ...
      if (readBack === null || !equalBytes(readBack, bytes)) return;
    } catch {
  ...
  async #publishReceipt(options: {
    ...
    if (this.#receiptStore === undefined) return;
...
function parsePayloadReference(value: unknown): CheckpointPayloadReference {
...
function parseAdapterSpec(value: unknown): CheckpointAdapterSpec {
  const record = requireRecord(value, "adapter");
  requireExactKeys(record, ["name", "version", "resume_mode"], []);
  const result: CheckpointAdapterSpec = {
...
function validateAdapterSpec(value: CheckpointAdapterSpec): void {
...
function parseBoundary(value: unknown): CheckpointBoundary {
...
export function parseCheckpointReference(value: unknown): CheckpointReference {
...
function requirePayloadPath(value: unknown): string {
  ...
  if (
    ...
    throw new Error(
      "checkpoint payload path must be a safe relative POSIX path",
    );
  }
  return path;
}

function requireObjectPath(value: unknown, name: string): string {
...
function requireJsonRecord(
  ...
  if (!isJsonRecord(canonical)) throw new Error(`${name} must be an object`);
  return canonical;
}
...
function requirePositiveInteger(value: unknown, name: string): number {
  ...
  if (result < 1) throw new Error(`${name} must be a positive safe integer`);
  return result;
}
...
function requireResumeMode(value: unknown): CheckpointResumeMode {
  if (
    value !== "exact" &&
    value !== "boundary" &&
    value !== "restart" &&
...
function parseJson(raw: Uint8Array, name: string): unknown {
  try {
...
function equalReference(
  left: CheckpointReference,
  right: CheckpointReference,
): boolean {
  return (
...
function equalAdapter(
  left: CheckpointAdapterSpec,
  right: CheckpointAdapterSpec,
): boolean {
  return (
